...
int main(int argc, char* argv[]) {
  // integer variables
  int i, j;
  int fdim = 2000;  // 2000 lines in the permittivity data
  ...
  d[0] = 0.;
  d[1] = 0.0007;  // 7 angstrom thickness
  d[2] = 0.;
...
*/
  ...
  FILE *rfpr, *rfpi;
  rfpr = fopen("../July6/2-atom_epsilon/epsr.dat","r");
  rfpi = fopen("../July6/2-atom_epsilon/epsi.dat","r");
  thetaI = 0.;
  double en, crap, epsr, epsi;
  double complex epsilon, ri;
  double *enl, *epsrl, *epsil;
  epsrl = (double *)malloc(2000*sizeof(double));
  epsil = (double *)malloc(2000*sizeof(double));
  enl   = (double *)malloc(2000*sizeof(double));
  ...
  for (i=0; i<fdim; i++) {
     ...
     // get wavelength in nm from energy in eV
     lambda = 1240./enl[i];

     // Wavenumber in inverse micrometers
     k0 = 2*pi*1000./lambda;
   
     // get complex epsilon from real and imaginary parts
     epsilon = epsrl[i] + epsil[i]*I;

     // Get square root of epsilon for ri
     ri = csqrt(epsilon);

     // Make sure we return the principle branch (positive real and im part)
     rind[1] = fabs(creal(ri)) + I*fabs(cimag(ri));
     ...
     fprintf(fp," %12.10f  %12.10f  %12.10f %12.10f  %12.10f\n",lambda,d[1]*1e3,refl, Trans,1-refl-Trans);
  }
  fprintf(fp,"\n");
  }
fclose(fp);
