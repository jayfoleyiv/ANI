...
*/
  ...
  FILE *rfpr, *rfpi;
  rfpr = fopen("epsr.dat","r");
  rfpi = fopen("epsi.dat","r");
  thetaI = 0.;
  double en, crap, epsr, epsi;
  double complex epsilon, ri;
  fprintf(fp,"# Energy (eV)  Wavelength (nm)  Reflectance  Transmittance  Absorbance    Re(epsilon)       Im(epsilon)\n");
  for (i=0; i<fdim; i++) {

     // read data from epsilon file
     fscanf(rfpr,"%lf",&en);
     fscanf(rfpr,"%lf",&epsr);
     fscanf(rfpr,"%lf",&crap);
     fscanf(rfpr,"%lf",&crap);
     ...
     fprintf(fp,"  %12.10f %12.10f  %12.10f %12.10f  %12.10f  %12.10f  %12.10f\n",
     en,lambda,100*refl, 100*Trans,100*(1-refl-Trans),epsr, epsi);
  }
